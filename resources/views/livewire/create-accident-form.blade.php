...
@section('content')
<div class="full-wd">
   <h3 class="create-accident-title">Νέα Εγγραφή [ΑΤΥΧΗΜΑ]</h3>

   <form wire:submit.prevent='save' class="data-input-form" enctype="multipart/form-data">
    @csrf
    {{-- @if($step == 0) --}}
        ...
        <div class="form-group s100-100">
            <label for="accidentImages">Φόρτωση Εικόνων</label>
            <input type="file" name="accidentImages" multiple="multiple" value="Αναζήτηση εικόνων">
            @error('accidentImages')
            <small class="error">{{$message}}</small>
            @enderror
        </div>
        </div>
